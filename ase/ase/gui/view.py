...
viridis = cm.get_cmap('turbo', 1000)
...
class View:
    ...
    def get_colors(self, rgb=False):
        ...
        if self.colormode == 'gcn':
            return [matplotlib.colors.rgb2hex(viridis(Z)) for Z in self.get_color_scalars()]
    ...
    def get_color_scalars(self, frame=None):
        ...
        if self.colormode == 'force':
            ...
            from ase.neighborlist import NeighborList
            n = len(self.atoms)
            nl = NeighborList(self.get_covalent_radii(self.atoms) * 1.2,
                              skin=0, self_interaction=False, bothways=True)
            nl.update(self.atoms)
            return [len(nl.get_neighbors(i)[0]) for i in range(n)]

        elif self.colormode == 'gcn':
            from ase.neighborlist import neighbor_list, natural_cutoffs, NeighborList
            self.atoms.set_pbc([1,1,0])
            n = len(self.atoms)

            nat_cut = natural_cutoffs(self.atoms, mult=1.2)
            ...
            for ndx in range(n):
                cni = 0
                indices, offsets = nl.get_neighbors(ndx)
                for j in indices:
                    x,_ =  nl.get_neighbors(j)
                    cn_j = len(x)/cn_max
                    cni=cni+cn_j

                gcn.append(cni)

            gcn = [round(i,3) for i in gcn]
            gcn = np.array(gcn)
            gcn = gcn/np.max(gcn)
            gcn = list(gcn)
            return gcn
